...
const PlanCard = ({
  ...
  isCurrentPlan,
  currency,
  subscriptionShortUrl // Passed from Plans.jsx
}) => {
  const getStorageDisplay = (storageValue) => {
    const val = parseFloat(storageValue);
    if (isNaN(val)) return "Custom Storage";
    ...
    if (isCurrentPlan) {
      return (
        <>
          ...
          <button
            className="bg-gradient-to-r from-blue-500 to-purple-600 opacity-60 cursor-not-allowed mt-4 font-medium text-sm text-center py-2 px-4 rounded shadow-lg"
            disabled
          >
            Your current plan
          </button>
          ...
          <button
            className="bg-gradient-to-r from-blue-500 to-purple-600 text-white mt-2 font-medium text-sm py-2 px-4 rounded shadow-lg hover:opacity-90 focus:ring-4 focus:ring-primary-200 dark:focus:ring-primary-900"
            onClick={() => {
              console.log("subscriptionShortUrl", subscriptionShortUrl);
              if (subscriptionShortUrl) {
                window.open(subscriptionShortUrl[0].short_url, "_blank");
              }
            }}
          >
        ...
        <div className="flex justify-center items-baseline mt-8">
          {isStatic ? (
            ...
            <span className="mr-2 text-4xl font-extrabold">
              {parseFloat(price) === 0 ? "Free" : currency === "USD" ? "$" : "₹"}
        ...
        <ul role="list" className="mb-2 space-y-4 text-left">
          {team_size && (
            <li className="flex items-center space-x-3">
              <Icon icon="ic:baseline-check" className="flex-shrink-0 w-5 h-5 text-green-500 dark:text-green-400" />
              <span>
          ...
          {storage && (
            <li className="flex items-center space-x-3">
              <Icon icon="ic:baseline-check" className="flex-shrink-0 w-5 h-5 text-green-500 dark:text-green-400" />
              <span>
                Storage size: <span className="font-semibold">{getStorageDisplay(storage)}</span>
              </span>
            </li>
          )}
          <li className="flex items-center space-x-3">
