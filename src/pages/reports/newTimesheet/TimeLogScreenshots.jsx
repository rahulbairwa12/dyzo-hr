...
const TimeLogScreenshots = () => {
    ...
    useEffect(() => {
        const fetchScreenshots = async () => {
            ...
            if (!taskId || !userInfo?._id) return;
            
            try {
                setLoading(true);
                const url = `${djangoBaseURL}/screenshot/task/${taskId}/employee/${userInfo?._id}/?page=${currentPage}&startDate=${startDate}&endDate=${endDate}&ordering=${ordering}&page_size=${limit}&userId=${selectedUserId}`;
        ...
        fetchScreenshots();
    }, [taskId, selectedUserId, currentPage, startDate, endDate, ordering]);
                                ...
                                <div key={index} className="bg-white dark:bg-slate-900 rounded-lg overflow-hidden shadow-sm border border-gray-200 dark:border-slate-700 p-2">
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex items-center justify-between text-xs gap-2">
                                            <div className="text-sm font-bold truncate" title={screenshot.project_name}>{screenshot.project_name}</div>
                                            {
                                                screenshot?.createdTime && 
                                                <div className="flex items-center gap-1 w-20">
                                                    <Icon icon="cuida:calendar-outline" className="w-4 h-4" />
                                                    <span>{moment.utc(screenshot?.createdTime).format('DD/MM/YYYY')}</span>
                                                </div>
                                            }
                                        </div>
                                        <div className="flex items-center justify-between text-xs text-gray-600 dark:text-gray-400">
                                                <ProfileCardWrapper userId={screenshot.userId} >
                                                <div className='flex gap-1 items-center'>
                                                    <ProfilePicture
                                                        user={{
                                                            name: screenshot.employee_name,
                                                            image: screenshot.employee_profile_pic ? `${screenshot.employee_profile_pic}` : null,
                                                        }}
                                                        className="w-7 h-7 rounded-full border-2 border-white"
                                                    />
                                                    <span className='truncate cursor-default'>{screenshot?.employee_name}</span>
                                                </div>
                                            </ProfileCardWrapper>
                                            {
                                                screenshot?.createdTime && 
                                                <div className="flex items-center gap-1 w-20">
                                                    ...
                                                    <span>{moment.utc(screenshot?.createdTime).format('HH:mm:ss')}</span>
                                                </div>
                                            }
                                        </div>
...
export default TimeLogScreenshots;