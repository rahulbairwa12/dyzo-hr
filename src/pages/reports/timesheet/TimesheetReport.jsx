...
const TimesheetReport = () => {
  ...
  const [CurrentUserId, setCurrentUserId] = useState(
    urlParams.get("employeeId") || (userInfo?.isAdmin ? "0" : userInfo?._id)
  );
  ...
  useEffect(() => {
    ...
    if (userInfo && employeeLists.length === 0) {
      fetchEmployee();
    }
  }, [userInfo, djangoBaseURL, employeeLists.length]);
    ...
    if (employeeLists && employeeLists.length > 0) {
      if (!isAdmin()) {
        if (userInfo?.team_leader) {
          ...
          if (CurrentUserId === "0") {
            setCurrentUserId(employeeLists[0]._id);
          }
        }
      }
    }
  }, [employeeLists, isAdmin(), userInfo, CurrentUserId]);


  let userOptions = employeeLists?.map((user) => ({
  ...
  //   }
  //   fetchTasksData(url);
  // }, [selectedProject, CurrentUserId, startDate, endDate]);
  ...
  const handleSearchQuery = (e) => {
    ...
    updateURLParams({ searchQuery: query });
  };
