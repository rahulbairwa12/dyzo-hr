      ...
      Cell: (row) => (
        ...
        <div className="flex space-x-2">
          {row?.cell?.value.map((employee, empIndex) => (
            <>
              <div key={empIndex} className="group">
                {employee.profilePicUrl ? (
                  <div className="flex items-center">
                    <img
                      src={`${djangoBaseURL}${employee.profilePicUrl}`}
                      alt={employee.name}
                ...
                <div className="absolute z-10 invisible inline-block w-64 text-sm text-gray-500 transition-opacity duration-300 bg-white border border-gray-200 rounded-lg shadow-sm opacity-0 group-hover:visible group-hover:opacity-100 dark:text-gray-400 dark:border-gray-600 dark:bg-gray-800 left-4/5 transform -translate-x-1/2 -translate-y-full mb-2">
                  <div className="px-3 py-2 bg-gray-100 border-b border-gray-200 rounded-t-lg dark:border-gray-600 dark:bg-gray-700">
                    <h3 className="font-semibold text-gray-900 dark:text-white text-sm">{employee.name}</h3>
                  </div>
                  <div className="px-3 py-2">
                    <p>{employee.totalTimeSpent}</p>
                  </div>
          ...
          <div className="flex space-x-3 rtl:space-x-reverse">
            ...
            <Tooltip content="Task Screenshots" placement="top" arrow animation="shift-away">
              <Link
                ...
                to={screenshotAllow ? `/timesheet/task-logs/${task._id}/${CurrentUserId}` : `/timesheet/task-logs/${task._id}/${CurrentUserId}`}
                state={{ task: task, filters: { selectedProject, CurrentUserId, startDate, endDate, selectedTimePeriod, selectedClient }, filterType: filterType }}
                className=""
                onClick={(e) => e.stopPropagation()}
              >
                ...
                >
                  {loading || dataLoading ? (
                    ...
                    rows.map((row) => {
                      prepareRow(row);
                      return (
                        <tr {...row.getRowProps()} className="cursor-pointer relative" onClick={() => { navigate(`/tasks?taskId=${row.original._id}`) }}>
                          {row.cells.map((cell) => (
