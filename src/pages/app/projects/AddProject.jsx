...
const AddProject = ({ showAddProjectModal, setShowAddProjectModal, fetchProjects }) => {
  ...
  useEffect(() => {
    const fetchEmployeeList = async () => {
      try {
        ...
        // Filter only active employees where isClient is false
        const onlyEmployees = data
          .filter(emp => emp.isActive && !emp.isClient) // Filter active employees who are not clients
          .map((emp) => ({
            value: emp._id,
            label: emp.name,
  ...
  useEffect(() => {
    const fetchClientList = async () => {
      try {
        const { data } = await fetchGET(`${import.meta.env.VITE_APP_DJANGO}/api/clients/by_company/${userInfo.companyId}/`)
        const options = data.map((emp) => ({ value: emp._id, label: emp.clientName, image: null }))
        setClient(options)
      } catch (error) {
      }
    }
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    isMulti={false}  // Assuming you want only one client to be selectable
                    onChange={(option) => field.onChange(option ? option.value : '')}  // Submit only the client's ID
                    value={client.find(option => option.value === field.value)}
                    id="icon_s"
                  />
                )}
              />
