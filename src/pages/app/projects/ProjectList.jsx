...
const ProjectList = ({ projects }) => {
  ...
  const COLUMNS = [
    ...
    },
    {
      ...
      accessor: "dateAdded",
      Cell: (row) => {
        return <span>{DateWithMonthName(row?.cell?.value)}</span>;
      },
    },
    {
      Header: "End Date",
      accessor: "dueDate",
      Cell: (row) => {
        return <div>{DateWithMonthName(row?.cell?.value)}</div>;
      },
    },
    {
      Header: "assignee",
          ...
          <div>
            <div className="flex justify-end sm:justify-start lg:justify-end xl:justify-start -space-x-1 rtl:space-x-reverse">
              ...
              {row?.cell?.value?.length === 0 ? ("No Assigne") : (
                ...
                row?.cell?.value?.slice(0, 2)?.map((user, userIndex) => (
                  <div
      ...
      Cell: (row) => {
        ...
        return (
          <div className="flex space-x-2 items-center">
            <span onClick={() => navigate('/project-details', {state : {project : row.row.original}})} ><Icons icon="hugeicons:view" className='w-6 h-6 cursor-pointer' /></span>
            <span ><Icons icon="heroicons:pencil-square" className='w-6 h-6 cursor-pointer' /></span>
            <span ><Icons icon="ph:trash-light" className='w-6 h-6 cursor-pointer' /></span>
          </div>
        );
      },
    },
