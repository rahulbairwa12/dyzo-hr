...
import { fetchAuthPatch, fetchGET, fetchPOST, fetchAuthGET } from "../../store/api/apiSlice";
...
const Toggle = ({ checked, onChange, disabled, label, description }) => {
  return (
    <label className={`flex items-start justify-between gap-4 py-2 cursor-pointer ${disabled ? 'opacity-60 cursor-not-allowed' : ''}`}>
      ...
      <span className="relative inline-flex items-center">
        ...
        <span
          aria-hidden
          className={`h-6 w-10 rounded-full transition-colors duration-200 ${checked ? 'bg-violet-600' : 'bg-slate-300 dark:bg-slate-600'}`}
        >
          <span
            className={`block h-5 w-5 rounded-full bg-white shadow transform transition-transform duration-200 translate-y-0.5 ${checked ? 'translate-x-5' : 'translate-x-0.5'}`}
          />
        </span>
      </span>
    </label>
...
const NotificationSettings = () => {
  ...
  const [initialSettings, setInitialSettings] = useState({}); // To store settings from backend or last saved
  ...
  // Config-driven fields grouped by section
  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      try {
        setLoading(true);
        const data = await fetchAuthGET(`${import.meta.env.VITE_APP_DJANGO}/notification-settings/${userInfo?._id}/`, false);
        if (!cancelled) {
          if (data?.error) {
            setError("Failed to load settings");
          } else {
            setSettings(data?.data || {});
            setInitialSettings(data?.data || {}); // Store initial settings
            setError("");
          }
        }
      } catch (e) {
  ...
  const sendWhatsAppOtp = async () => {
    try {
      setSendingOtp(true);
      const url = `${import.meta.env.VITE_APP_DJANGO}/api/whatsapp/send-otp/${userInfo?._id}/`;
  ...
  const verifyWhatsAppOtp = async () => {
    try {
      ...
      const url = `${import.meta.env.VITE_APP_DJANGO}/api/whatsapp/verify-otp/${userInfo?._id}/`;
      ...
      if (data?.status === 0) {
        ...
        // Update userInfo in auth state and cookies
        try {
          dispatch(login({ ...(userInfo || {}), whatsapp_verified: true }));
        } catch (_) { }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetchAuthPatch(
        `${import.meta.env.VITE_APP_DJANGO}/notification-settings/${userInfo?._id}/`,
        { body: settings }
      );
    ...
    return (
      <div className="bg-white rounded-md p-4 min-h-[80vh]">
        <div className="border border-neutral-50 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-md p-4">
          ...
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-10 bg-slate-100 dark:bg-slate-700/50 rounded mb-2 animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-white rounded-md p-4 min-h-[80vh]">
      <ToastContainer />
      <form onSubmit={handleSubmit} className="space-y-4 rounded-md p-4">
        <div className="flex items-center justify-between">
          <h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-slate-900">Notification Settings</h2>
          <button
            type="submit"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {sections.map((section) => (
            <div key={section.title} className="border border-neutral-50 dark:border-slate-700 rounded-md p-3 bg-white">
                  ...
                  <>
                    <Toggle
                      ...
                      onChange={(e) => {
                        ...
                        const isWaVerified = !!(settings.whatsapp_verified || (userInfo && userInfo.whatsapp_verified));
                        if (next) {
                          if (isWaVerified) {
                      ...
                      description={(settings.whatsapp_verified || (userInfo && userInfo.whatsapp_verified)) ? "Verified" : "Verification required"}
                    />
                    ...
                    <Toggle
                      ...
                      disabled={saving || !(settings.whatsapp_verified || (userInfo && userInfo.whatsapp_verified))}
                      label="Daily task summary"
                    />

                    {(settings.whatsapp_verified || (userInfo && userInfo.whatsapp_verified)) && (
        ...
        {/* OTP Modal */}
        {otpModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black-500/40">
