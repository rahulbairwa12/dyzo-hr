...
import DatePickerStartAndEndDate from './DatePickerStartAndEndDate';
...
const UserScreenshots = () => {
    ...
    const fetchScreenshots = async (page) => {
        try {
            setLoading(true);
            const response = await fetchGET(`${import.meta.env.VITE_APP_DJANGO}/api/employee/${userId}/screenshots/${startDate}/${endDate}/?page=${page}&ordering=${order}`);
            if (response.results && response.results.status) {
                setScreenshots(prevScreenshots => [...prevScreenshots, ...response.results.data]);
    ...
    useEffect(() => {
        ...
        setHasMore(true);
        fetchScreenshots(1);
    }, [startDate, endDate, userId, order]);
    ...
    const screenshotByDate = Object.groupBy(screenshots, ({ dateCreated }) => dateCreated.split('T')[0]);
                                    ...
                                    <div className='grid grid-cols-1 md:grid-cols-3 gap-4 py-2'>
                                        {screenshotByDate[date].length > 0 ? (
                                            screenshotByDate[date].map((item, i) => {
                                                const imageIndex = screenshots.findIndex(screenshot => screenshot._id === item._id);
                                                return (
                                                    <ScreenShotCard
