...
import { topMenu, topMenuEmployee, clientMenuItems } from "@/constant/data";
...
const HorizentalMenu = () => {
  ...
  useEffect(() => {
    const checkUerType = () => {
      ...
      if (userInfo?.user_type === 'employee' && userInfo?.isAdmin) {
        setSidebarMenuItem(topMenu);
      }
      else if (userInfo?.user_type === 'employee' && userInfo?.team_leader) {
        setSidebarMenuItem(topMenu);
      } else if (userInfo?.user_type === 'employee' && !userInfo?.isAdmin && !userInfo?.team_leader) {
        setSidebarMenuItem(topMenuEmployee);
      } else {
        ...
        {sidebarMenuItem?.map((item, i) => (
          ...
          >
            ...
            {!item.child && !item.megamenu && (
              <Link to={item.link!='tasks' ? item.link : `${item.link}?userId=${userInfo._id}` }>
